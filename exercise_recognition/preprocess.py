...
def preprocess_frames(loc):
    ...
    np.save(os.path.join(loc, "res", "X_exercises"), arr=X)
    np.save(os.path.join(loc, "res", "y_exercises"), arr=y)
...
def load_train_data(loc):
    # Load numpy arrays
    X = np.load(os.path.join(loc,"res","X_exercises.npy"))
    y = np.load(os.path.join(loc,"res","y_exercises.npy"))
