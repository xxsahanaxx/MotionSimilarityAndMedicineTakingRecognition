...
RNN_type = folder_setup.train_types[1]
...
history_logger=CSVLogger(folder_setup.os.path.join(loc,"res",'loss'+RNN_type+'.csv'), separator=",", append=True)
...
def define_model(model_type):
    if model_type == "LSTM": 
        ...
        model.add(LSTM(64, return_sequences=True, activation='relu', input_shape=(folder_setup.sequence_length,258)))
        ...
        model = Sequential()
        # Input shape => 258 co-ordinates across 30 frames
        model.add(GRU(units=64, return_sequences=True, activation='relu', input_shape=(folder_setup.sequence_length,258)))
...
if __name__ == "__main__":
    ...
    if train == True:
        ...
        if(RNN_type == "LSTM"):
            ...
            open(folder_setup.os.path.join(folder_path,'lossLSTM.csv'), mode='w').close()
            model = train_model(model, 'LSTMexercises.h5', os.path.join(loc,"models"))
        # If the model is a GRU one
        elif(RNN_type == "GRU"):
            # Open the respective loss CSV file and clear it 
            open(folder_setup.os.path.join(folder_path,'lossGRU.csv'), mode='w').close()
