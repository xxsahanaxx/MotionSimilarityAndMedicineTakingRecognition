...
def probability_wizard(actions, input_frame, colors, results1, results2=None):
    ...
    for num, prob in enumerate(results1):
        ...
        cv2.putText(output_frame, actions[num]+" ({:.2f})".format(prob), (0, 85+num*40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
    if results2 is not None: 
        for num, prob in enumerate(results2):
            cv2.rectangle(output_frame, (640,60+num*40), (int(640+prob*300), 90+num*40), colors[num], -1)
            cv2.putText(output_frame, actions[num]+" ({:.2f})".format(prob), (640, 85+num*40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2, cv2.LINE_AA)
    return output_frame
...
model performances using metrics to the console.
    The per sample predictions and true results are stored inside a CSV file 
    called 'testPred<LSTM/GRU>.csv'.

    Inputs: loc: location where the train and test data is saved
            model: the defined model (LSTM/GRU) with loaded weights 
            RNN_type: whether the model is "LSTM" or "GRU" 
'''
...
A sequence constantly re-initialises itself to only hold the last 30 frames of 
    ...
    Inputs: model1: loaded weights model (either LSTM/GRU)
            model2: loaded weights model (either GRU/LSTM) for comparison purposes
    ...
    with mediapipe_interpreter.mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while cam.isOpened():
            ...
            if len(sequence) == 30:
                ...
                # Save predictions made in a CSV
                # with open(folder_setup.os.path.join(loc,"res",'predictionsmodel1.csv'), 'a', encoding='UTF8',newline='') as f:
                #     writer = csv.writer(f)
                ...
                if(model2 is not None):
                    ...
                    # with open(folder_setup.os.path.join(loc,"res",'predictionsmodel2.csv'), 'a', encoding='UTF8',newline='') as f:
                    #     writer = csv.writer(f)
            ...
            cv2.rectangle(image, (0,0), (380, 40), (178, 163, 221), -1)
