...
seventy_five_percent_scale_videos = folder_setup.no_videos*2
hundred_twenty_five_percent_scale_videos = folder_setup.no_videos*3
...
def scale_dataset(scale1=0.75, scale2=1.25):
    ...
    for action in folder_setup.exercises:
        ...
        for sequence in range(1,folder_setup.no_videos+1):
            try: 
                os.makedirs(os.path.join(folder_setup.DATA_PATH, action, str(sequence+seventy_five_percent_scale_videos)), exist_ok=True)
            except:
                pass
            ...
            for frame_num in range(folder_setup.sequence_length):
                ...
                np.save(folder_setup.os.path.join(folder_setup.DATA_PATH, action, str(sequence+seventy_five_percent_scale_videos), str(frame_num)), new_res)
                cv2.imwrite(folder_setup.os.path.join(folder_setup.DATA_PATH, action, str(sequence+seventy_five_percent_scale_videos),str(frame_num)+".jpg"),img)
        
        print("Scaled coordinates for video {} by 0.75".format(sequence))
    ...
    for action in folder_setup.exercises:
        ...
        for sequence in range(1,folder_setup.no_videos+1):
            try: 
                os.makedirs(os.path.join(folder_setup.DATA_PATH, action, str(sequence+hundred_twenty_five_percent_scale_videos)), exist_ok=True)
            except:
                pass
            ...
            for frame_num in range(folder_setup.sequence_length):
                ...
                # Save as a new frame in same action
                np.save(folder_setup.os.path.join(folder_setup.DATA_PATH, action, str(sequence+hundred_twenty_five_percent_scale_videos), str(frame_num)), new_res)
                cv2.imwrite(folder_setup.os.path.join(folder_setup.DATA_PATH, action, str(sequence+hundred_twenty_five_percent_scale_videos),str(frame_num)+".jpg"),img)
        
        print("Scaled coordinates for video {} by 1.25".format(sequence))
